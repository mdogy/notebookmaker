...
baby = pd.read_csv(
    "https://raw.githubusercontent.com/data-8/textbook/main/assets/data/baby.csv"
)
...
print(
    f"Is {observed_delta_mu:.3f} oz surprisingly large, or could it happen by chance?"
)
...
perm_nonsmoker = baby_permuted[baby_permuted["Maternal Smoker"] == False][
    "Birth Weight"
]
...
plt.hist(
    ...
    color="lightblue",
    edgecolor="black",
    label="Permuted differences (null distribution)",
)
plt.axvline(
    observed_delta_mu,
    color="red",
...
plt.axvline(
    0, color="gray", linestyle="-", linewidth=1, alpha=0.5, label="Zero difference"
)

plt.xlabel("Difference in Mean Birth Weight (Non-smoker - Smoker) [oz]", fontsize=12)
plt.ylabel("Frequency", fontsize=12)
plt.title(
    "Permutation Test: Null Distribution of Birth Weight Differences",
    fontsize=14,
    fontweight="bold",
)
...
print("\n📌 Interpretation:")
print("   If smoking had NO effect, we'd see a difference this large")
print("   or larger in 5% of random shuffles.")
